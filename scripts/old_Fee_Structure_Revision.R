...
#Memo as of Jan 26. Need to fix a column with a name like "fees doubled" because not generalizable. 
#Also there must be a miscoding. When setting increase in soz fee to -0.1, the fare for SOZ becomes 0.3, which is incorrect. 
...
trips_per_zone_fy21to22_SOZ_double <- trips_per_zone_fy21to22 %>% mutate(trip_fee_soz_doubled = case_when(
  ...
  `Trip Origin and Destination` == "Out_of_Bound - MDD" ~ 0.06,
  `Trip Origin and Destination` == "Out_of_Bound - Out_of_Bound" ~ 0,
  `Trip Origin and Destination` == "Out_of_Bound - SOZ" ~ 0.40+increase_in_soz_fee,
  `Trip Origin and Destination` == "Out_of_Bound - SPD" ~ 0.20,
  `Trip Origin and Destination` == "SOZ - EFMDD" ~ 0,
  `Trip Origin and Destination` == "SOZ - MDD" ~ 0.06,
  `Trip Origin and Destination` == "SOZ - Out_of_Bound" ~ 0.40+increase_in_soz_fee,
  `Trip Origin and Destination` == "SOZ - SOZ" ~ 0.40+increase_in_soz_fee,
  `Trip Origin and Destination` == "SOZ - SPD" ~ 0.20,
  `Trip Origin and Destination` == "SPD - EFMDD" ~ 0,
  `Trip Origin and Destination` == "SPD - MDD" ~ 0.06,
  `Trip Origin and Destination` == "SPD - Out_of_Bound" ~ 0.20,
...
trips_per_zone_fy21to22_EFMDD_subsidized <- trips_per_zone_fy21to22_EFMDD_subsidized %>%
  mutate(new_EFMDD_ridership = Total * change_in_EFMDD_ridership) #Calculating increased ridership due to subsidy

trips_per_zone_fy21to22_EFMDD_subsidized <- trips_per_zone_fy21to22_EFMDD_subsidized %>%
  mutate(cost_for_subsidy = new_EFMDD_ridership * fee_reduction)

total_cost_for_subsidy <- sum(trips_per_zone_fy21to22_EFMDD_subsidized$cost_for_subsidy)

#####################################################################
###Operator's Revenues After Fee Revision
#####################################################################
trips_per_zone_fy21to22_operator_revenue_post_fee_rev_temp <- trips_per_zone_fy21to22_SOZ_double %>%
...
trips_per_zone_fy21to22_operator_revenue_post_fee_rev_temp <- trips_per_zone_fy21to22_operator_revenue_post_fee_rev_temp %>%
...
trips_per_zone_fy21to22_operator_revenue_post_fee_rev <- trips_per_zone_fy21to22_operator_revenue_post_fee_rev_temp %>%
  left_join(trips_per_zone_fy21to22_EFMDD_subsidized, by = "Trip Origin and Destination") %>%
  select(-new_ridership, -Total, -fee_reduction, -cost_for_subsidy) %>%
  mutate(final_new_ridership = ifelse(is.na(new_ridership2), new_EFMDD_ridership, new_ridership2)) %>% #Finalized how much ridership in each group
  select(-new_ridership2, -new_EFMDD_ridership) %>% #Deleted unnecessary columns
  mutate(new_operator_revenue = fare*final_new_ridership) %>%
  filter(`Trip Origin and Destination` != "Out_of_Bound - Out_of_Bound") #Excluded out-of-bound - out-of-bound trips
rm(trips_per_zone_fy21to22_operator_revenue_post_fee_rev_temp) #Removed temporary data frame

operator_revenue_post_fee_rev <- sum(trips_per_zone_fy21to22_operator_revenue_post_fee_rev$new_operator_revenue)

#####################################################################
###Operator's Revenue Status Quo
#####################################################################
operator_revenue_status_quo <- fare_status_quo*sum(trips_per_zone_fy21to22_status_quo$Total)

#####################################################################
### Compare Operator's Post/Status Quo Revenue
#####################################################################
print(operator_revenue_post_fee_rev)
print(operator_revenue_status_quo)
print(operator_revenue_post_fee_rev-operator_revenue_status_quo) #want this to be above 0. Want to say that operator's revenue increased. 

#####################################################################
###Compare cost for subsidy and increase in revenue for LADOT
#####################################################################
print(total_cost_for_subsidy)
print(rev_increase_soz_doubled)
print(rev_increase_soz_doubled+total_cost_for_subsidy) #Want this to be 0 or above. We don't want LADOT's fee revenue to decrease. 
